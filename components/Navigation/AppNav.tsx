...
export default function AppNav({ showAuthButton = false, transparent = false }: AppNavProps) {
  ...
  // Bottom nav items (6 most important for mobile)
  const bottomNavItems = [
    navItems[1], // Timer
    navItems[2], // Track (StudyTrack)
    navItems[3], // Pod
    navItems[4], // Plan/Calendar
    navItems[5], // Stats/Analytics
        ...
        <div className="grid grid-cols-6 h-16">
          {bottomNavItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`flex flex-col items-center justify-center gap-0.5 py-1.5 transition-all active:scale-95 ${
                isActive(item.href)
                  ? 'text-primary'
                  : 'text-gray-500 dark:text-gray-400'
              }`}
            >
              <div className={`relative flex items-center justify-center w-8 h-6 rounded-lg transition-all ${
                isActive(item.href)
                  ? 'bg-primary/10'
                  : ''
              }`}>
